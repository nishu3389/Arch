...
import com.three.u.image_picker.ImagePickerActivity
...
//        intent.putExtra(ImagePickerActivity.INTENT_ASPECT_RATIO_Y, 1)
    ...
    private fun navigationWork(view: WebView? ,url: String?) {
        if (url.toString().contains("returnStripe", true))
            navigate(R.id.SuccessBankAccountFragment)
        else if (url.toString().contains("reauthStripe", true)) {
            mBinding.webView.gone()
            mViewModel.callGetBankSetupDetailApi().observe(viewLifecycleOwner, Observer {
                reloadFragment(it.data?.verificationLink)
            })
        }
        else {
            /*if (url!!.contains(".pdf")) {
